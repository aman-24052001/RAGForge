...
private:
    ...
    std::unordered_map<std::string, std::vector<size_t>> inverted_index_; // term → chunk ids
    ...
    std::vector<std::string> tokenize_query(const std::string& q) const;
    void update_bm25_index(size_t id, const std::string& text);
};
